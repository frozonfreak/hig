---
trigger: always_on
description: Enforce The Web HIG on UI and front-end work
---

# The Web HIG (agent contract)

Before writing or changing UI, CSS, or front-end architecture:
...
3. For **content**, resolve **surface** from scope: `document` (default) | `hybrid` | `experience` (**HIG-EXP-001**); preload `rules/expressive-surface.md` when surface is hybrid or experience.
4. Apply **Layer 1** default context: `docs/hig/HIG-QUICK.md` (or repo `HIG-QUICK.md`) — follow The Web HIG Quick Reference.
...
7. On topic match, load Layer 2 module from `docs/hig/rules/manifest.yaml` (e.g. `rules/accessibility.md`). Load `framework/*.md` when stack-specific.
8. Escalate to **Layer 3** full spec (`docs/hig/HIG.md` or repo `HIG.md`) only for edge cases or conflicts.
9. Apply the Layer 0 mandatory/optional matrix from `rules/applicability.md`.
